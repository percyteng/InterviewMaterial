...
//Use HEAP whenever you need quick access to the largest (or smallest) item, because that item will always be the first element in the array or at the root of the tree.
//
...
//
//If you think of a heap as a binary tree stored in linear order by depth, with the 
//root node first (then the children of that node next, then the children of those 
//nodes next); then the children of a node at index N are at 2N+1 and 2N+2. This property allows quick access-by-index. And since heaps are manipulated by swapping nodes, this allows for in-place sorting.

//queue point: front element indicator, rear element indicator, number of elements
//buffer(array), add, remove and peek.
public class queue {
